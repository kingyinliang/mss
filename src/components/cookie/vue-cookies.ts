...
const VueCookies: VueCookiesType = {
  ...
  set: function set(key: string, value: any, expireTimes?: string | number | Date, path?: string, domain?: string, secure?: boolean, sameSite?: string) {
    if (!key) {
      ...
      switch (expireTimes.constructor) {
        ...
        case String:
          if (/^(?:\d+(y|m|d|h|min|s))$/i.test(expireTimes as string)) {
            ...
            let _expireTime = (expireTimes as string).replace(/^(\d+)(?:y|m|d|h|min|s)$/i, '$1') ;
